window.MAP=(function(){
  ...
  function buildNCROutline(){
    ...
    // Glow layer
    el('path',{d:delhiPath,fill:'none',stroke:'#1B3D6B','stroke-width':'3',
      opacity:'.4',filter:'url(#glow-ncr)','pointer-events':'none'},gNCR);
    // Main border line
    el('path',{d:delhiPath,fill:'rgba(10,30,58,0.18)',stroke:'#2A5FA8',
      'stroke-width':'1.5','stroke-dasharray':'8,5',opacity:'.65','pointer-events':'none'},gNCR);

    // District/area subtle fill zones
    // Gurgaon zone (SW)
    el('path',{d:'M 200,830 L 60,760 L -30,650 L -50,480 L 60,550 L 100,680 Z',
      fill:'rgba(0,150,180,0.04)',stroke:'rgba(0,190,212,0.2)','stroke-width':'1',
      'stroke-dasharray':'5,6',opacity:'.7','pointer-events':'none'},gNCR);
    // Noida zone (SE-E)
    el('path',{d:'M 820,500 L 860,390 L 980,380 L 1060,560 L 1060,700 L 980,820 L 860,870 L 780,700 Z',
      fill:'rgba(0,150,120,0.04)',stroke:'rgba(0,200,150,0.2)','stroke-width':'1',
      'stroke-dasharray':'5,6',opacity:'.7','pointer-events':'none'},gNCR);
    // Ghaziabad zone (NE)
    el('path',{d:'M 660,-8 L 840,20 L 900,120 L 820,140 L 760,52 Z',
      fill:'rgba(180,100,0,0.03)',stroke:'rgba(220,140,0,0.18)','stroke-width':'1',
      'stroke-dasharray':'5,6',opacity:'.7','pointer-events':'none'},gNCR);
    // Faridabad zone (S)
    el('path',{d:'M 640,820 L 720,780 L 780,700 L 860,870 L 700,870 L 540,850 Z',
      fill:'rgba(160,0,100,0.03)',stroke:'rgba(200,0,130,0.18)','stroke-width':'1',
      'stroke-dasharray':'5,6',opacity:'.7','pointer-events':'none'},gNCR);

    // City/region labels
    const lblStyle={'font-family':'Space Mono,monospace','pointer-events':'none',
    ...
    txt(0,-16,'N',{'font-family':'Space Mono,monospace','font-size':'7',fill:'#3E5272',
      'text-anchor':'middle'},cg);
    ...
    txt(0,22,'S',{'font-family':'Space Mono,monospace','font-size':'7',fill:'#3E5272',
      'text-anchor':'middle'},cg);
  }
  ...
  function buildLines(){
    Object.entries(METRO.LINE_PATHS).forEach(([key,ids])=>{
      ...
      if(!pts)return;
      // Shadow/glow pass
      el('polyline',{points:pts,fill:'none',stroke:col,'stroke-width':'6','stroke-linecap':'round',
        'stroke-linejoin':'round',opacity:'.12',filter:'url(#glow-soft)','data-line':lc},gLines);
      // Main line
      const pl=el('polyline',{points:pts,fill:'none',stroke:col,'stroke-width':'3.5',
        'stroke-linecap':'round','stroke-linejoin':'round',opacity:'.9','data-line':lc},gLines);
      lineEls[key]=pl;
    });
  }

  function lineCode(key){
    // Exact match only — avoids P→Pink being confused with M→Magenta
    // (no startsWith; all LINE_PATH keys are fully enumerated here)
    const map={
      Y:'Y',
  ...
  function buildStations(){
    Object.entries(METRO.STATIONS).forEach(([id,s])=>{
      ...
      if(s.ix)el('circle',{cx:s.x,cy:s.y,r:r+5,fill:'none',stroke:'#fff',
        'stroke-width':'.7',opacity:'.15'},g);
      if(s.hot){
        // Pulse ring
        const pr=el('circle',{cx:s.x,cy:s.y,r:r+10,fill:'none',stroke:'#FF1744',
      ...
      const c=el('circle',{cx:s.x,cy:s.y,r,fill:'#00E676',stroke:'#080C12',
        'stroke-width':s.ix?1.8:1,'data-cid':id},g);
      markerEls[id]=c;
  ...
  // ── Labels ────────────────────────────────────────────────────────
  // tier 0 = hot stations  → visible at sc>1.4
  // tier 1 = interchanges  → visible at sc>1.9
  // tier 2 = normal        → visible at sc>2.8
  function buildLabels(){
    Object.entries(METRO.STATIONS).forEach(([id,s])=>{
      const tier=s.hot?0:s.ix?1:2;
      const fill=s.hot?'#FF6B6B':s.ix?'#B0C4DE':'#7A9BBE';
      // Use full name, truncated at 18 chars
      const label=s.n.length>18?s.n.slice(0,16)+'…':s.n;
      const t=el('text',{
        x:s.x+8, y:s.y+4,
        'font-size':'6',
  ...
  function setupInput(){
    ...
    w.addEventListener('mousedown',e=>{_md=true;dragging=false;dx=e.clientX;dy=e.clientY;
    ...
    w.addEventListener('touchstart',e=>{
      if(e.touches.length===1){_md=true;dragging=false;dx=e.touches[0].clientX;
  ...
  function apply(){
    ...
    // Tiered label visibility:
    //  sc < 1.4 → no labels (too crowded)
    //  sc 1.4–2.4 → hot/interchange stations only
    //  sc > 2.4 → all labels
    if(gLbl){
      const labels=gLbl.querySelectorAll('text');
      labels.forEach(t=>{
        const tier=parseInt(t.dataset.tier||'2');
        const show=(tier===0&&sc>1.4)||(tier===1&&sc>1.9)||(tier===2&&sc>2.8);
        t.setAttribute('opacity',show?'1':'0');
        // Scale font size counter to zoom so text stays readable but not huge
        const fs=Math.max(5,Math.min(9,6/sc));
  ...
  function updateMarkers(){
    ...
    Object.entries(METRO.STATIONS).forEach(([id,s])=>{
      ...
      const isClosed=v===0;
      c.setAttribute('fill', isClosed ? '#1C2A3C' : CROWD.color(v));
      c.setAttribute('stroke', isClosed ? '#2A3F58' : 'none');
      c.setAttribute('stroke-width', isClosed ? '1' : '0');
      c.setAttribute('opacity', show ? (isClosed ? '.45' : '1') : '.06');
      c.setAttribute('r', isClosed ? '5' : (v>=81?8:v>=61?7:6));
    });
